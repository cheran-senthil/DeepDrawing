...
from keras.layers import Dense, Reshape, Flatten, Conv2D, Input
from keras.layers import BatchNormalization, Activation, Dropout, ZeroPadding2D
...
if backend.image_data_format() == 'channels_first':
    real_images = np.reshape(real_images, (shape[0], shape[3], shape[1], shape[2]))
else:
    real_images = np.reshape(real_images, (shape[0], shape[1], shape[2], shape[3]))
...
#    imgs = np.append(imgs, gen_imgs, axis=0)
    ...
    pbar.set_description("[D loss: %.3f, acc.: %.2f%%] [G loss: %.3f]" % (d_loss[0], 100*d_loss[1], g_loss))
